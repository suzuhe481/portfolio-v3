...
export const Timeline = ({ data }: { data: IExperienceDataProps[] }) => {
  ...
  return (
    <div className="w-full font-sans px-8 lg:px-36 pt-8" ref={containerRef}>
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          <motion.div
            ...
            whileInView={{ y: 0 }}
            viewport={{ once: true }}
            className="flex justify-start py-2 lg:py-4 lg:gap-8 bg-white/70 rounded-2xl my-2 lg:my-4"
          >
            <div className="lg:sticky flex flex-col lg:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm lg:w-2/3">
              {/* Bullet point */}
              <div
                ref={index === data.length - 1 ? lastCircleRef : null}
                className="h-10 absolute left-3 lg:left-3 w-10 rounded-full bg-white  flex items-center justify-center"
              >
                <div className="h-4 w-4 rounded-full bg-neutral-200 border border-neutral-300 p-2" />
              </div>

              {/* Desktop Title */}
              <h3 className="hidden lg:block text-2xl lg:text-4xl lg:pl-20 font-bold text-black">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-16 pr-4 lg:pl-4 w-full">
              {/* Mobile Title */}
              <h3 className="lg:hidden block text-2xl mb-2 text-left font-bold text-black underline">
                {item.title}
              </h3>
              {/* Negative top margin to align markdown with bullet point */}
              <div className="markdown flex flex-col justify-start mt-[-8px] font-geist-mono text-base lg:text-lg">
        ...
        <div
          ...
          className="absolute lg:left-8 left-8 top-0 overflow-hidden w-[4px] bg-[linear-gradient(to_bottom,var(--tw-gradient-stops))] from-transparent from-[0%] via-neutral-200 dark:via-neutral-700 to-transparent to-[99%]  [mask-image:linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)] "
        >
          <motion.div
            style={{
